...
export default {
  ...
  methods: {
    ...
    onChangeCheckbox(event) {
      ...
      if (!event.target.checked) {
        this.multi_selector = this.multi_selector.filter(
          item => item !== event.target.value
        );
        this.input_values = this.input_values.filter(
    ...
    validate() {
      this.$v.$touch();
      if (!this.$v.multi_selector.required)
        this.errText = `Выберите хотя бы один из вариантов в поле "${this.inp_placeholder}"`;
